...
export function workspaceFromIntent(intent: string, display: string, fallback: WorkspaceId): WorkspaceId {
  const joined = `${intent} ${display}`
  return workspaceFromText(joined, fallback)
}
