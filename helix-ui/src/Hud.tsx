...
import type { AudioMetrics, AudioMetricsRef, CountdownState, HelixState, NativeTelemetry, TerminalLog, ThemeSpec, WeatherTelemetry } from './types'
  ...
  return (
    <div className="helix-shell h-screen overflow-hidden text-white" style={style}>
      ...
      <main className="relative z-10 grid h-[calc(100vh-58px)] min-h-0 grid-cols-1 grid-rows-[minmax(0,1fr)_minmax(190px,30vh)] gap-2 p-2 xl:grid-cols-12 xl:grid-rows-[minmax(0,1fr)_230px] xl:gap-3 xl:p-4">
        <aside className="hidden min-h-0 xl:col-span-3 xl:block"><Telemetry theme={theme} metrics={props.metrics} data={props.telemetry} weather={props.weather} /></aside>
...
function Status({ mode, theme, telemetry, bridgeReady }: { mode: HelixState; theme: ThemeSpec; telemetry: NativeTelemetry; bridgeReady: boolean }) {
  return <header className="relative z-20 flex h-[58px] items-center justify-between border-b border-white/8 bg-black/25 px-3 backdrop-blur-xl md:px-6">
    <div className="flex min-w-0 items-center gap-3">
      <div className="grid size-8 place-items-center border font-mono text-[9px]" style={{ borderColor: `rgba(${theme.rgb}/.55)`, color: theme.hex, boxShadow: `inset 0 0 15px rgba(${theme.rgb}/.15)` }}>FR</div>
      <div className="min-w-0"><p className="truncate font-mono text-[11px] tracking-[.2em] text-white/90 md:text-sm">F.R.I.D.A.Y. // HELIX</p><p className="truncate font-mono text-[7px] tracking-[.22em] text-white/32 md:text-[9px]">SEONGJA PRIVATE INTELLIGENCE // BUILD 0.9.21</p></div>
    </div>
    <div className="flex items-center gap-3 font-mono text-[7px] tracking-[.16em] text-white/45 md:text-[9px]">
      <span className="hidden md:inline">{telemetry.time} // {telemetry.voiceSource.toUpperCase()} // {bridgeReady ? 'NATIVE LINK' : 'SYNC'}</span>
...
function CoreStage(props: Props & { theme: ThemeSpec }) {
  const { mode, theme, metrics, telemetry, countdown, weather } = props
  return <section className="core-stage relative min-h-0 overflow-hidden border xl:col-span-6" style={{ borderColor: `rgba(${theme.rgb}/.3)`, boxShadow: `inset 0 0 80px rgba(${theme.rgb}/.055),0 0 40px rgba(${theme.rgb}/.08)` }}>
    <button type="button" aria-label="Reset FRIDAY voice input" onClick={props.onCoreTap} className="absolute inset-x-0 top-0 bottom-20 z-20 cursor-crosshair bg-transparent" />
    ...
    <div className="absolute inset-x-3 bottom-3 z-30 border border-white/10 bg-black/65 px-3 py-2 backdrop-blur-md">
      ...
      <div className="mt-2 min-w-0 font-mono text-[7px] tracking-[.12em] text-white/40">
        <p className="truncate">HEARD // {props.transcript}</p>
        <p className="mt-1 truncate" style={{ color: theme.hex }}>VOICE LINK // {telemetry.voiceSource.toUpperCase()} // PRIVATE CHANNEL</p>
      </div>
...
function WeatherChip({ weather, theme, onRefresh }: { weather: WeatherTelemetry; theme: ThemeSpec; onRefresh: () => void }) {
  if (!weather.configured) return <div className="w-full border border-white/8 px-2 py-1.5 font-mono text-[7px] tracking-[.12em] text-white/32">WEATHER CORE // INITIAL SETUP PENDING</div>
  const stale = !weather.fresh
  return <button type="button" aria-label="Refresh FRIDAY weather intelligence" onClick={onRefresh} className="grid w-full grid-cols-[auto_minmax(0,1fr)_auto] items-center gap-2 border border-white/8 px-2 py-1.5 text-left font-mono text-[7px] tracking-[.1em]" style={{ color: weather.alert ? '#fca5a5' : theme.hex }}>
    <span className="text-sm">{weather.icon || weatherGlyph(weather.condition)}</span>
    <span className="min-w-0 truncate">{Math.round(weather.tempC)}°C // {weather.condition.toUpperCase()} // FEELS {Math.round(weather.feelsLikeC)}° // WIND {Math.round(weather.windKph)} KPH // RAIN {weather.rainChance}%</span>
    <span className="text-white/30">{stale ? 'STALE' : weather.location.toUpperCase().slice(0,18)}</span>
  </button>
}
...
function Telemetry({ theme, metrics, data, weather }: { theme:ThemeSpec; metrics:AudioMetrics; data:NativeTelemetry; weather:WeatherTelemetry }) {
  const values = [['DEVICE',data.device],['BATTERY',`${data.battery}%`],['NETWORK',data.network],['HEAP',`${data.heapMb} MB`],['CLOCK',data.time],['CORTEX',data.cloudConfigured?'MESH ONLINE':'LOCAL MODE'],['WEATHER',weather.configured?`${Math.round(weather.tempC)}°C ${weather.condition}`:'PENDING'],['RAIN',weather.configured?`${weather.rainChance}%`:'--']]
  return <Panel title="TELEMETRY" eyebrow="NATIVE + WEATHER LIVE SIGNAL" theme={theme}><div className="grid grid-cols-2 gap-2">{values.map(([label,value])=><div key={label} className="border border-white/6 bg-white/[.025] p-3"><p className="font-mono text-[8px] tracking-[.18em] text-white/32">{label}</p><p className="mt-2 truncate font-mono text-[11px] text-white/82">{value}</p></div>)}</div><div className="mt-5 space-y-4"><Meter label="RMS / VOICE" value={metrics.rms} theme={theme}/><Meter label="PEAK ENVELOPE" value={metrics.peak} theme={theme}/><Meter label="CORE ENERGY" value={Math.min(1,metrics.rms*.75+theme.energy*.25)} theme={theme}/></div><p className="mt-5 border-l-2 pl-3 font-mono text-[8px] leading-5 tracking-[.11em] text-white/42" style={{ borderColor:theme.hex }}>FRIDAY USES VERIFIED NATIVE STATE AND ENCRYPTED WEATHER DATA, NEVER DECORATIVE GUESSES.</p></Panel>
}
