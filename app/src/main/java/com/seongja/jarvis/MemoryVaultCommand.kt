...
object MemoryVaultCommandParser {
    fun parse(raw: String): MemoryVaultCommand? {
        ...
        val namesVault = Regex("\\b(memory vault|memory center|memory settings|saved memories)\\b")
        ...
        val search = Regex(
            "(?:find|search|show)\\s+(?:my\\s+)?(?:memory vault|saved memories)\\s+(?:for|about|containing)\\s+(.+)"
        ).find(clean)
