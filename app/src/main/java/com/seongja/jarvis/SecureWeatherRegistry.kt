...
/** Stores the WeatherAPI key and coordinates under Android Keystore AES-GCM. */
class SecureWeatherRegistry(context: Context) {
    ...
    fun load(): WeatherSettings {
        ...
        if (raw.isBlank()) return WeatherSettings()
        return runCatching {
            ...
            WeatherSettings(
                ...
                refreshMinutes = root.optInt(
                    "refreshMinutes",
                    WeatherSettings.DEFAULT_REFRESH_MINUTES
                ).coerceIn(
    ...
    fun save(settings: WeatherSettings) {
        val payload = JSONObject().apply {
            ...
            put("latitude", settings.latitude ?: JSONObject.NULL)
            put("longitude", settings.longitude ?: JSONObject.NULL)
            put("enabled", settings.enabled)
            put("refreshMinutes", settings.refreshMinutes.coerceIn(
                WeatherSettings.MIN_REFRESH_MINUTES,
                WeatherSettings.MAX_REFRESH_MINUTES
            ))
        ...
        prefs.edit().putString(KEY_SETTINGS, encrypt(payload)).apply()
    }

    @Synchronized
    fun recordSuccess(latencyMs: Long, statusCode: Int) {
        val current = load()
        save(
            current.copy(
    ...
    companion object {
        private const val PREFS_NAME = "jarvis_weather_encrypted"
        private const val KEY_SETTINGS = "weather_api"
        private const val KEY_ALIAS = "jarvis_weather_key_v1"
